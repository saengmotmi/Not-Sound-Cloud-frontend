...
const CanvasComp = ({
  ...
  useEffect(() => {
    ...
    if (metaData && music.src && state.nowSongID === metaData.song_id) {
      if (state.isPlaying && state.isPause === false) {
        ...
        let interval = 820 / duration;

        wavePlay = setInterval(() => {
          console.log("go interval");
          setOffsetX(offsetX + interval);
          setFunc.setNowSec(
            parseInt(music.src.buffer.duration * (offsetX / 820) + 1)
          );
  ...
  const onCanvasClick = (e, pos) => {
    ...
    if (offsetClick < 820) {
      if (pos === "top" && offsetClick < 820) {
        const secCalc =
          music.src &&
          parseInt(music.src.buffer.duration * (offsetClick / 820));
  ...
  return (
    <CanvasContainer>
      ...
      <OverDivWrapper playPos={offsetX} mouseOn="y" widthProps={`${mousePos}`}>
        <OverDiv ref={canvasRefOver2} width="820" height="100" />
      </OverDivWrapper>
      <OverDivWrapper mouseOn="n" widthProps={`${mousePos}`}>
        <OverDiv ref={canvasRefOver1} width="820" height="100" />
      </OverDivWrapper>
      {commentArr}
    </CanvasContainer>
  );
