...
import {drawWave} from "../../global/func";
import VisualComment from '../Visualizer/VisualizerComment';
...
const VisualizerComp = (props) => {
  ...
  const getComment = () => {
    ...
    fetch("http://10.58.1.163:8000/comment?song_id=3", {
      method: "GET",
      headers: myHeaders
    })
      .then(res => res.json())
...
const sendComment = () => {
  ...
  myHeaders.append("Content-Type", "application/json");
  myHeaders.append("Authorization", token);
  fetch("http://10.58.1.163:8000/comment", {
    method: "POST",
