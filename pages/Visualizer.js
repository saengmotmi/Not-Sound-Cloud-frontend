...
const Visualizer = () => {
  ...
  useEffect(() => {
    ...
    for (j of dataUpscale) {
      ctxTop.fillRect(i, 100, 2.2, -300 * j); // x, y 시작점, width, height
  ...
  return (
    <>
      <div style={{ position: 'relative', height: '200px' }}>
        {/* <Canvas onClick={onCanvasClick} width="0" height="100" ref={canvasRef1} /> */}
        <Canvas posId="wave-top" onMouseMove={onCanvasMove} onClick={(event) => onCanvasClick(event, 'top')} width="640" height="100" ref={canvasRefTop} />
        <Canvas posId="wave-bot" onMouseMove={onCanvasMove} onClick={(event) => onCanvasClick(event, 'bot')} width="640" height="100" ref={canvasRefBot} />
      ...
      {/* <button type="button" onClick={turnModalOn}>회원가입</button> */}
      {/* <button type="button" onClick={() => { console.log(canvasRef); }}>ref</button> */}
      {/* <SignModal isVisible={modalVisible} /> */}
      {/* <FileUpload /> */}
    </>
  );
};
...
const Canvas = styled.canvas`
  position: absolute;
  overflow: hidden;
  top: ${(props) => (props.posId === 'wave-top' ? 0 : 100)}px;
`;
